...
int main(void) {
  ...
  while (-1 != (line_size = getline(&buffer, &buffer_size, stdin))) {
    ...
    int shown[3] = {0};
    while (i + 1 < line_size) { // skip newline
      if (buffer[i] == ',' || buffer[i] == ';') {
        ++i;
      }
      ++i;
      int number;
      sscanf(buffer + i, "%d", &number);
      while (buffer[i++] != ' ')
        ;

      int *v = shown + index[(int)buffer[i]];
      i += skip[(int)buffer[i]];
      *v = number > *v ? number : *v;
    }
    ++i;
    total_power += shown[0] * shown[1] * shown[2];
  }
  printf("%zd", total_power);
  free(buffer);
  return 0;
}
